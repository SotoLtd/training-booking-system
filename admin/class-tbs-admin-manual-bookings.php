...
class TBS_Admin_Manual_Bookings {
	/**
	 * Admin handler
	 * @var obj
	 */
	...
	public function maybe_load_list_table(){
		$current_action = $this->get_current_action();
		if(!in_array( $current_action, array('edit', 'view_email_records') )){
			$this->set_list_table('bookings');
			$this->list_table->set_base_url(self::url());
			$this->list_table->set_booking_type('manual');
		...
		switch( $this->get_current_action() ){
			case 'edit': 
				if(!empty($_GET['booking_id'])){
					$args['new_booking'] = '<a href="' . self::url('edit') . '" class="page-title-action">Add Booking</a>'; 
	...
	public function ajax_generate_online_url(){
		...
		$unique_key = uniqid();
		update_post_meta($order_id, '_tbs_online_form_id', $unique_key);
		wp_send_json(array(
			'status' => 'OK',
			'onlineFormUrl' => add_query_arg(array('booking_manual_key' => $unique_key), site_url()),
		));
	}
	/**
	...
	public function ajax_save_booking(){
		...
		if(!$data_entry_complete || $suppress_order_emails){
			...
			$this->disable_wc_new_order_email();
		}
		// Create or get order
		if(!$order_id){
			$order = wc_create_order(array(
		...
		$response['status'] = 'OK';
		$response['ID'] = $order->get_id();
		$response['editUrl'] = self::url( 'edit', array( 'booking_id' => $order->get_id() ) );
		wp_send_json($response);
	}
	/**
	...
	public function ajax_load_booking(){
		...
		if($online_form_key){
			$booking_data['onlineFormUrl'] = add_query_arg(array('booking_manual_key' => $online_form_key), site_url());
		}
		
		// Get Address
		$address = array();
		foreach ( self::get_address_fields() as $key => $field ) {
		...
		foreach($line_items as $line_item_id => $line_item){
			...
			$item_data['subtotal'] = (float)$line_item->get_subtotal();
			$item_data['total'] = (float)$line_item->get_total();
			$item_data['coursePermalink'] = TBS_Admin_Course_Date_Info::url('view', array('course_date_id' => $course_date->get_id(),));
			$prepeared_items[] = $item_data;
		}
	...
	public function disable_wc_new_order_email(){
		add_filter('woocommerce_email_enabled_new_order', '__return_false', 20);
	}
	public function enable_wc_new_order_email(){
		remove_filter('woocommerce_email_enabled_new_order', '__return_false', 20);
	}
	/**
	...
	public static function url($action = '', $extra_query_args = array()){
		$query_args = array('page' => 'tbs-manual-bookings');
		if($action){

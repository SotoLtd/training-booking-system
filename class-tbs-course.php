...
class TBS_Course {
	...
	public function get_dates($args = array()){
		...
		while($date_query->have_posts()){
			$date_query->the_post();
			$course_dates[] = new TSB_Course_Date( get_the_ID());
		}
		wp_reset_postdata();
		return $course_dates;
	}
	...
	public function add_date($args){
		...
		if(!$start_date || !$end_date || !$price || !$trainer){
			return false;
		}
		$start_date_format = date('F d Y', $start_date);
		$end_date_format = date('F d Y', $end_date);
		$product_name = $start_date_format;
		$product_slug = date('Ymd', $start_date);
		if($end_date != $start_date){
			$product_name .= ' - ' . $end_date_format;
			$product_slug = date('Ymd', $end_date);
		}
		...
		if($prduct_id = $product->save()){
			return new TSB_Course_Date($prduct_id);
		}
		return false;
	}

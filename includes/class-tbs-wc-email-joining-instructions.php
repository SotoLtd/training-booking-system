...
if ( ! class_exists( 'TBS_WC_Email_Joining_Instructions', false ) ) :
/**
 ...
 *
 * @class       TBS_WC_Email_Joining_Instructions
 * @version     2.0.0
 * @package     WooCommerce/Classes/Emails
 * @author      WooThemes
 * @extends     WC_Email
 */
class TBS_WC_Email_Joining_Instructions extends WC_Email {
	public $course_ji_data = false;
	/**
	 * Constructor.
	 */
	public function __construct() {
		$this->id             = 'joining_instructions';
		$this->customer_email = true;
		$this->title          = __( 'Joining instructions', TBS_i18n::get_domain_name() );
		$this->description    = __( 'Joining instructions are sent to bookders and delegates email addresses when a booker completes an order', TBS_i18n::get_domain_name() );
		$this->template_html  = 'emails/joining-instructions.php';
		$this->template_plain = 'emails/plain/joining-instructions.php';
	...
	public function get_delegates_email_data(){
		...
		foreach($order_delegates as $course_date_id => $d_ids){
			...
			$delegates_emails = array($this->object->get_billing_email());
			foreach ($d_ids as $d_id){
				$delegate = new TBS_Delegate($d_id);
				if(!$delegate->exists()){
					continue;
				}
				if($delegate->has_email()){
			...
			$data[$course_date_id] = array(
				...
				'course_title' => $course_date->get_course_title(),
				'course_date_title' => $course_date->get_course_title_with_date(),
				'course_formatted_date' => $course_date->get_date_formatted(),
	...
	public function set_ji_data($course_ji_data){
		...
		$this->placeholders['{course_title}'] = $course_ji_data['course_title'];
		$this->placeholders['{course_date}'] = $course_ji_data['course_formatted_date'];
	}
	...
	public function get_attachments() {
		$attachments = array();
		$map_url = $this->course_ji_data['course_date']->get_map();
		if($map_url){
...
return new TBS_WC_Email_Joining_Instructions();
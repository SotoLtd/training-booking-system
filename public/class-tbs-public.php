...
class TBS_Public {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'tts-course-js', $this->get_assets_url( 'js/frontend.js' ), array(
			'jquery',
			'tipped',
			'datatables'
		) );
	...
	public function sc_course_dates( $atts, $content = "" ) {
		...
		ob_start();
		tbs_get_template_part( 'course-dates-list', true, array(
			'course_dates' => $course_dates,
			'show_private' => $query_args['show_private']
		) );
	...
	public function coupon_is_valid_for_courses( $valid, $product, $coupon, $values ) {
		$course_date_ids = tbs_get_course_dates_for_coupon( $coupon->get_id() );

		if ( in_array( $product->get_id(), $course_date_ids, true ) || in_array( $product->get_parent_id(), $course_date_ids, true ) ) {
			$valid = true;
		}else{
			$valid = false;
		}
	...
	public function coupon_apply_quantity( $qu, $item ) {
		var_dump($item->price);
		var_dump($qu);
		return $qu;
	}
	...
	public function checkout_field_validation() {
		...
		$delegates_email_fields     = array_filter( $checkout_fields, array(
			$this,
			'checkout_delegates_email_fields_filter'
		) );
	...
	public function wc_manage_order_delegates_on_status_change( $order_id, $status_from, $status_to, $order ) {
		...
		if ( in_array( $status_to, array(
				'cancelled',
				'refunded',
				'failed'
			) ) && ! in_array( $status_from, array( 'cancelled', 'refunded', 'failed' ) ) ) {
		...
		if ( in_array( $status_from, array(
				'cancelled',
				'refunded',
				'failed'
			) ) && ! in_array( $status_to, array( 'cancelled', 'refunded', 'failed' ) ) ) {
